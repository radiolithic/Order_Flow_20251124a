...
class ShopifyInventoryImporter:
    ...
    def download_inventory_export(self):
        ...
        if response.status_code != 200:
            self.info(f"HTTP Error {response.status_code} starting Shopify export: {response.text}")
            self.info("\nPossible causes:")
            self.info("  - Invalid access token")
            self.info("  - Incorrect shop URL")
            self.info("  - API access not enabled for this app")
            return None

        # Check for GraphQL errors
        response_data = response.json()
        if 'errors' in response_data:
            self.info(f"GraphQL errors: {response_data['errors']}")
            return None

        # Check for user errors
        if response_data['data']['bulkOperationRunQuery']['userErrors']:
            errors = response_data['data']['bulkOperationRunQuery']['userErrors']
            self.info(f"User errors starting Shopify export: {errors}")
            for error in errors:
                self.info(f"  - {error.get('field', 'Unknown field')}: {error.get('message', 'Unknown error')}")
            return None
        ...
        # Give the bulk operation a moment to initialize
        time.sleep(2)
        ...
        while True:
            ...
            if data['status'] == 'COMPLETED':
                ...
                self.info(f"\nShopify bulk operation {data['status']}")
                if data.get('errorCode'):
                    self.info(f"Error code: {data['errorCode']}")

                    # Provide specific guidance based on error code
                    if data['errorCode'] == 'ACCESS_DENIED':
                        self.info("\nACCESS_DENIED means:")
                        self.info("  - Your Shopify custom app needs additional permissions")
                        self.info("  - Go to Shopify Admin > Settings > Apps and sales channels")
                        self.info("  - Select your custom app > Configuration")
                        self.info("  - Enable these Admin API scopes:")
                        self.info("    * read_products")
                        self.info("    * read_inventory")
                        self.info("  - Save and reinstall the app")
                    else:
                        self.info("\nThis typically means:")
                        self.info("  1. The Shopify API query is too complex for your store's data")
                        self.info("  2. Your Shopify plan doesn't support bulk operations")
                        self.info("  3. The API access token lacks required permissions")

                self.info("\nWORKAROUND: Use manual CSV export instead:")
                self.info("  1. In Shopify Admin, go to Products > Inventory")
                self.info("  2. Click 'Export' and download as CSV")
                self.info("  3. Save the file to: shared-data/input/inventory_export_latest.csv")
                self.info("  4. Re-run this script - it will automatically use the CSV file")
                return None
    ...
    def import_inventory(self, csv_file=None):
        ...
        If download fails, automatically fall back to most recent local CSV file.
        """
        if csv_file is None:
            csv_file = self.download_inventory_export()
            if csv_file is None:
                self.info("Failed to download inventory export from Shopify.")
                self.info("Checking for existing CSV files as fallback...")
                csv_file = self.find_latest_inventory_export()
                if csv_file is None:
                    self.info("\nNo local CSV files found. Unable to continue.")
                    self.info("Please manually export inventory from Shopify and place it in shared-data/input/")
                    return None
                else:
                    self.info(f"Using fallback CSV file: {csv_file}")
